...
/ long_window_4dvar_utils.py, once ported into this repo) needs from a
forecast-model backend, so a future backend (a different checkpoint or
architecture) can be swapped in without touching the solver itself.

Copied verbatim (2026-09-16) from the AIFS v2 port's own forecast_model.py --
see that repo's CLAUDE.md "Isolating AIFS-specific code behind a model
...
(see the AIFS repo's CLAUDE.md "control_space: 'latent'" section for why the
earlier physical-space control path was removed there). A backend with no
accessible latent representation between its encoder and decoder is out of
scope entirely, not a case this interface degrades gracefully for -- there
is deliberately no physical-space fallback method here.

WHAT THIS DOES NOT COVER: historical IC/verification fetching (ERA5/CDS) is
deliberately a SEPARATE concern from model architecture -- see
InitialConditionProvider below, sketched but not fully worked out.
aifs_grid.GridInterpolator (copied here as fcn3_grid.GridInterpolator) also
needs no change at all: it's already backend-agnostic (a k-d tree over
whatever (lons, lats) it's given), so it isn't part of this interface -- any
backend's `.lats`/`.lons` feed it directly.
"""
...
class ModelState(Protocol):
    ...
    `AIFSState.state` happens to be two packed lagged time levels; FCN3's
    `FCN3State.state` is a single physical-space time level (no history) --
    as long as that SAME backend's own `advance`/`decode_state` can
    round-trip it, the solver core never indexes into `.state` directly,
    only through `decode_state()`, so this stays opaque by design. `.date`
    is load-bearing: `compute_optimal`'s analysis is only ever valid at
    `window_start + dt_verif` (the latent-injection mechanics below are
    why), so every consumer keys off `.date`, never an assumption that a
    state shares its background's date.
    """
...
class LatentForecastModel(abc.ABC):
    ...
    def latent_shape(self) -> tuple[int, ...]:
        """The shape compute_optimal's latent-space control increment must
        have. Required, not optional: every backend behind this interface
        is assumed latent-capable (see module docstring) -- there is no
        sentinel/None value meaning "no latent access", that case is simply
        out of scope. AIFS's is a 2-tuple (n_hidden_mesh_nodes, n_channels);
        FCN3's is a 3-tuple (n_channels, h, w) since its latent stays a
        spatial (not flattened-node) tensor -- callers must treat this as an
        opaque shape to allocate an increment of, not assume a fixed rank.
        """
    ...
    def decode_state(
        ...
        (KeyError on access) rather than fake a value, and the full solver
        port that would actually call decode_state() for this purpose
        hasn't been done yet in this repo.
        """
    ...
    def prepare_initial_state(self, input_state: dict, date: datetime.datetime) -> ModelState:
        ...
        (aifs_ic.py-equivalent, unwritten here yet) is expected to call
        this, not the solver core directly -- see InitialConditionProvider
        below."""
    ...
    def prime_from_state(self, state: ModelState) -> ModelState:
        ...
        calling prepare_initial_state -- see fcn3_model.FCN3Model's
        prime_from_state docstring for what that means for this backend's
        stochastic-noise state specifically)."""
    ...
    def advance(
        ...
        for AIFS; FCN3 re-embeds from the physical state on every step too,
        for the same reason -- see fcn3_model.FCN3Model.advance). A backend
        supporting genuine per-step latent persistence would need the
        solver core's injection-timing logic revisited too, not just this
        method -- compute_optimal / compute_loss_4dvar as they exist today
        (in the AIFS repo; not yet ported here) assume one-shot injection.
        """
