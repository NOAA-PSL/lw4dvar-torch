...
class AIFSModel(forecast_model.LatentForecastModel):
    ...
    def state_layout(self) -> str:
        """'channels_last': the packed state is `(1, multi_step, n_points,
        n_vars)` -- `n_vars` is the trailing dim, unlike FCN3Model's
        `(1, n_vars, H, W)` ('channels_first'). See FCN3Model.state_layout."""
        return "channels_last"

    def wrap_state(self, state: torch.Tensor, date: datetime.datetime) -> AIFSState:
        """Construct this backend's state wrapper -- lets solver-core code
        build a new state without importing/naming AIFSState directly."""
        return AIFSState(state, date)

    def _prime_noise(self) -> None:
        """No-op: AIFS has no stochastic input to re-prime (unlike FCN3's
        diffusion-noise state -- see FCN3Model._prime_noise). Exists so
        solver-core code (compute_optimal/compute_loss_4dvar) can call
        `model._prime_noise()` unconditionally on either backend."""
        pass
