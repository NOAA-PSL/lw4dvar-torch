...
parser.add_argument("--compile", action="store_true",
                     help="wrap the model in torch.compile (compile_wrapper=True)")
parser.add_argument("--n_calls", type=int, default=1,
                     help="repeat the call this many times on the SAME model instance, "
                          "to separate one-time compile overhead from steady state")
...
for i in range(args.n_calls):
    ...
    try:
        ...
        print(
            f"RESULT call={i+1}/{args.n_calls} steps={args.steps} "
            f"outer_checkpoint={not args.no_outer_checkpoint} compile={args.compile}: "
