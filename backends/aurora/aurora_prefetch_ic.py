"""
Warm the ERA5 IC/verification cache (aurora_ic.py) for a planned
long_window_4dvar.py run, BEFORE submitting the job to the (offline) H100
partition via run_aurora.sh.
...
nodes have none. Run this from a login node first:
    ...
    python -u backends/aurora/aurora_prefetch_ic.py
    sbatch run_aurora.sh

Fetches, in order:
  - (non-restart only) the initial "nhrs_back"-hours-before-sdate LAGGED
    state -- Aurora needs two time levels (`date - 6h` and `date`), same as
    AIFS, fetched via aurora_ic.build_input_state (which internally calls
    aurora_ic.read_single_date_fields twice and caches both) -- only needed
    once, for k=0's get_input() (subsequent cycles advance the previous
    analysis forward with the model instead of fetching a new IC)
  - one single-date verification state per DA cycle (get_verif() is called
    for every cycle/window, always at that cycle's `date`, via a single
    un-lagged aurora_ic.read_single_date_fields call)
...
Ported unchanged in structure from the FCN3 repo's fcn3_prefetch_ic.py --
only the initial-IC fetch (aurora_ic.build_input_state, which fetches TWO
lagged dates -- Aurora has n_history==1, i.e. two packed time levels, unlike
FCN3's single time level) and the module-level import differ.
"""
...
# lives at the repo root -- running it as `python backends/aurora/
# aurora_prefetch_ic.py` sets sys.path[0] to backends/aurora/ (Python's
...
# direct test, 2026-09-19 -- the same bug affects aifs_prefetch_ic.py and
# fcn3_prefetch_ic.py now that they've been copied into backends/{aifs,
# fcn3}/ instead of living at the repo root as in their single-backend
# source repos).
...
logger.info('loading Aurora checkpoint (device=cpu; only used for its variable/grid metadata here)...')
...
if not exp_config['restart']:
    ...
    aurora_ic.build_input_state(back_dt, cache_dir)
else:
...
for k in range(n_init):
    ...
    aurora_ic.read_single_date_fields(date_dt, cache_dir)
    for dv in dt_verifs:
        ...
        aurora_ic.read_single_date_fields(shifted_dt, cache_dir)
    date = utils.add_hours(date, exp_config['dt_init'])
